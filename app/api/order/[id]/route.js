...
export async function POST(req, { params }) {
	...
	console.log("[API PRODUCT POST]req:", productId, quantity);
	try {
		await connectToDB();
		const { _id, no } = await Seq.findOne({ _id: "order" });
		const product = await ProductModel.findOne({ _id: productId });
		console.log("[API PRODUCT POST] _id:", no);
		console.log("[API PRODUCT POST] product:", product);
		...
		console.log("[API PRODUCT POST] aProduct:", aProduct);
		const cost = { products: total, shippingFees: aProduct.shippingFees, total: totalOrd };
		console.log("[API PRODUCT POST] cost:", cost);
